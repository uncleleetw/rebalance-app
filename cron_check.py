...
# =========================================================================
# 🧠 第一大核心：總經加權風控塔台 (使用共用 df 且優化動態指標)
# =========================================================================
def get_risk_control_report(df):
    ...
    try:
        ...
        if len(hyg_series) >= 30:
            ...
            data['hy_oas'] = round(hyg_pct_30d, 2)  # 借用原本變數名避免大幅改動後續結構
    ...
    if is_monthly_check:
        try:
            w5000 = df['Close']['^W5000'].dropna().iloc[-1] if '^W5000' in df['Close'] else yf.Ticker("^W5000").history(period="5d")['Close'].dropna().iloc[-1]
    ...
    # 新判定邏輯：台幣匯率 30日均線偏離幅度 (暴貶代表資金大舉外流)
...
# =========================================================================
# 📊 第二大核心：資產再平衡決策哨兵 (使用共用 df 杜絕 Yahoo 二次封鎖)
# =========================================================================
def get_rebalance_
